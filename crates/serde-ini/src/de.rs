...
pub trait Trait {
    fn next(&mut self) -> Option<std::result::Result<Item, Error>>;
}

impl<E, T: Iterator<Item = std::result::Result<Item, E>>> Trait for T
where
    Error: From<E>,
{
    fn next(&mut self) -> Option<std::result::Result<Item, Error>> {
...
pub enum Error {
    ...
    #[error(transparent)]
    ParseStrError(#[from] parse::Error<std::convert::Infallible>),

    #[error(transparent)]
    ParseReadError(#[from] parse::Error<std::io::Error>),
    ...
    #[error("internal consistency error: unexpected EOF")]
    UnexpectedEof,

    /// Internal consistency error
    ///
    /// Encountering this is probably misuse of the deserialization API or a bug in serde-ini.
    #[error("internal consistency error: invalid state")]
    InvalidState,
}
...
impl<R: std::io::BufRead> Deserializer<parse::Parser<std::io::Lines<R>>> {
...
impl<R: std::io::Read> Deserializer<parse::Parser<std::io::Lines<std::io::BufReader<R>>>> {
...
impl<'a> Deserializer<parse::Parser<parse::OkIter<std::str::Lines<'a>>>> {
...
pub fn from_bufread<R, T>(reader: R) -> Result<T>
where
    R: std::io::BufRead,
    T: DeserializeOwned,
{
...
pub fn from_read<R, T>(reader: R) -> Result<T>
where
    R: std::io::Read,
    T: DeserializeOwned,
{
